...
namespace Package.EventManager.Implementation
{
    ...
    public class ServerPattern : MonoBehaviour
    {
        #region Variables
        ...
        {
            AddEvent(InternalTopics.RemoveTopics, TopicType.Event, false);
            AddEvent(InternalTopics.RemoveListenersFromTopics, TopicType.Event, false);
        }
        ...
        private void CustomDebug(LogType _type, params IConvertible[] _messages)
        {
            ...
            {
                Debug.LogError(CustomLibrary.UtilitiesGen.BuildString(_messages));
            }
            else if (_type.Equals(LogType.Warning))
            {
                Debug.LogWarning(CustomLibrary.UtilitiesGen.BuildString(_messages));
            }
            else if (_type.Equals(LogType.Log))
            {
                Debug.Log(CustomLibrary.UtilitiesGen.BuildString(_messages));
            }
        ...
        {
            if (_type.Equals(TopicType.Event)) SharedAddEvent<CustomTopic<A>>(GetTopicData(_topicName), _isPersistent); else SharedAddEvent<CustomFunctionTopic<A>>(GetTopicData(_topicName), _isPersistent);
        }

        public void AddEvent<A, B>(IConvertible _topicName, TopicType _type, bool _isPersistent)
        {
            if (_type.Equals(TopicType.Event)) SharedAddEvent<CustomTopic<A,B>>(GetTopicData(_topicName), _isPersistent); else SharedAddEvent<CustomFunctionTopic<A,B>>(GetTopicData(_topicName), _isPersistent);
        }

        public void AddEvent<A, B, C>(IConvertible _topicName, TopicType _type, bool _isPersistent)
        {
            if (_type.Equals(TopicType.Event)) SharedAddEvent<CustomTopic<A,B,C>>(GetTopicData(_topicName), _isPersistent); else SharedAddEvent<CustomFunctionTopic<A,B,C>>(GetTopicData(_topicName), _isPersistent);
        }

        public void AddEvent<A, B, C, D>(IConvertible _topicName, TopicType _type, bool _isPersistent)
        {
            if (_type.Equals(TopicType.Event)) SharedAddEvent<CustomTopic<A,B,C,D>>(GetTopicData(_topicName), _isPersistent); else SharedAddEvent<CustomFunctionTopic<A,B,C,D>>(GetTopicData(_topicName), _isPersistent);
        }

        public void AddEvent<A, B, C, D, E>(IConvertible _topicName, TopicType _type, bool _isPersistent)
        {
            if (_type.Equals(TopicType.Event)) SharedAddEvent<CustomTopic<A,B,C,D,E>>(GetTopicData(_topicName), _isPersistent); else SharedAddEvent<CustomFunctionTopic<A,B,C,D,E>>(GetTopicData(_topicName), _isPersistent);
        }
