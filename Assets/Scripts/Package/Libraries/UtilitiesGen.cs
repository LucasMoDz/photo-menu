...
namespace Package.CustomLibrary.Implementation
{
    public class UtilitiesGen : MonoBehaviour
    {
        ...
        {
            EventManager.ServerPattern.Invoke(EventManager.Implementation.InternalTopics.RemoveListenersFromTopics);
            EventManager.ServerPattern.Invoke(EventManager.Implementation.InternalTopics.RemoveTopics);

            EventManager.ServerPattern.AddEvent(EventManager.Implementation.InternalTopics.RemoveTopics);
            EventManager.ServerPattern.AddEvent(EventManager.Implementation.InternalTopics.RemoveListenersFromTopics);
        ...
        private static IEnumerator LoadSceneCO(IConvertible _sceneName, float _minSeconds, Button _buttonToDisable, LoadSceneMode _sceneMode, Image _imageToFill, Text _textToShow)
        {
            ...
            if (!Mathf.Approximately(_minSeconds, 0.0f))
            {
                ...
                if (_textToShow != null)
                {
                    UpdateText = _value => { _textToShow.text = CustomLibrary.UtilitiesGen.BuildString((int) (_value * 100), "%"); };
                }

                float step = 0;
                float temp_value = 0;
                float randomWait = _minSeconds + UnityEngine.Random.Range(-(_minSeconds / 5), _minSeconds / 5);
        ...
        public T[] Sort<T>(SortingOrder _order, params T[] _array) where T : IComparable
        {
            ...
            Debug.Log(CustomLibrary.UtilitiesGen.BuildString("Sorting time: ", string.Format("{0:0.000}", (Time.realtimeSinceStartup - beforeTime) * 1000), " ms\n"));
        ...
        private const string SAVE_DIRECTORY = "/Save/Data_";
        ...
        {
            DirectoryInfo folder = new DirectoryInfo(CustomLibrary.UtilitiesGen.BuildString(Application.persistentDataPath, SAVE_DIRECTORY, EventManager.ServerPattern.Invoke<int>(SaveVersionTopics.GetSaveVersion)));
            ...
            switch (_type)
            {
                case SaveType.JsonEncrypted:
                    streamWriter = new StreamWriter(CustomLibrary.UtilitiesGen.BuildString(Application.persistentDataPath, SAVE_DIRECTORY, EventManager.ServerPattern.Invoke<int>(SaveVersionTopics.GetSaveVersion), "/", _fileName, ".json"), false, Encoding.UTF8, 65536);
                ...
                case SaveType.Binary:
                    ...
                    FileStream file = File.Create(CustomLibrary.UtilitiesGen.BuildString(Application.persistentDataPath, SAVE_DIRECTORY, EventManager.ServerPattern.Invoke<int>(SaveVersionTopics.GetSaveVersion), "/", _fileName));
                ...
                case SaveType.TextEncrypted:
                    ...
                    streamWriter = new StreamWriter(CustomLibrary.UtilitiesGen.BuildString(Application.persistentDataPath, SAVE_DIRECTORY, EventManager.ServerPattern.Invoke<int>(SaveVersionTopics.GetSaveVersion), "/", _fileName, ".txt"), false, Encoding.UTF8, 65536);
            ...
            Debug.Log(CustomLibrary.UtilitiesGen.BuildString(_fileName, " has been saved\n"));
        }
        ...
        public T ReadingByFile<T>(FileName _fileName, SaveType _type)
        {
            ...
            switch (_type)
            {
                ...
                case SaveType.JsonDecrypted:
                    ...
                    filePath = CustomLibrary.UtilitiesGen.BuildString(Application.persistentDataPath, SAVE_DIRECTORY, EventManager.ServerPattern.Invoke<int>(SaveVersionTopics.GetSaveVersion), "/", _fileName, ".json");

                    if (!File.Exists(filePath))
                    {
                        Debug.Log(CustomLibrary.UtilitiesGen.BuildString(_fileName, " not found\n"));
                        return default(T);
                    }
                ...
                case SaveType.Binary:
                    ...
                    filePath = CustomLibrary.UtilitiesGen.BuildString(Application.persistentDataPath, SAVE_DIRECTORY, EventManager.ServerPattern.Invoke<int>(SaveVersionTopics.GetSaveVersion), "/", _fileName);
                    ...
                    Debug.Log(CustomLibrary.UtilitiesGen.BuildString(_fileName, " found, data has been loaded\n"));
                    return saveData;
                ...
                case SaveType.TextEncrypted:
                    ...
                    filePath = CustomLibrary.UtilitiesGen.BuildString(Application.persistentDataPath, SAVE_DIRECTORY, EventManager.ServerPattern.Invoke<int>(SaveVersionTopics.GetSaveVersion), "/", _fileName, ".txt");

                    if (!File.Exists(filePath))
                    {
                        Debug.LogWarning(CustomLibrary.UtilitiesGen.BuildString(_fileName, " not found\n"));
                        return default(T);
                    }
        ...
        public T EnumConverter<T>(IConvertible _value)
        {
            if (!typeof(T).IsEnum)
            {
                Debug.LogError(CustomLibrary.UtilitiesGen.BuildString(typeof(T).ToString(), " is not an enum!\n"));
                return default(T);
            }
            ...
            Debug.LogError(CustomLibrary.UtilitiesGen.BuildString("'", typeof(T).ToString(), "' not contains '", _value, "'!\n"));
            return default(T);
        }
        ...
        private const string ENCRIPTION_KEY = "Aj9lk!Bb0Xz";
        ...
        public string Encrypt(string _textToEncrypt)
        {
            ...
            {
                Rfc2898DeriveBytes pdb = new Rfc2898DeriveBytes(ENCRIPTION_KEY, SALT);
